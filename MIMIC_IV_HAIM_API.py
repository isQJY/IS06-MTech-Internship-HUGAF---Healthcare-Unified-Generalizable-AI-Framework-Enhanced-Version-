...
class Patient_ICU(object):
    def __init__(self, admissions, demographics, transfers, core,
        ...
        echonotes, radnotes, 
        # 新增数据模态
        genomic_data=None, wearable_data=None, pathology_images=None):
        ...
        self.wearable_data = wearable_data  # 可穿戴设备数据
        self.pathology_images = pathology_images  # 病理图像数据
...
# GET GENOMIC DATA EMBEDDINGS OF MIMIC IV PATIENT
def get_genomic_embeddings(genomic_data):
    """
    Extract genomic data embeddings
    
    Args:
        genomic_data: Genomic data DataFrame
        
    Returns:
        Genomic embeddings vector
    """
    # If no genomic data, return zero vector
    if genomic_data is None or len(genomic_data) == 0:
        return np.zeros(100)  # Assume 100-dimensional embedding
    
    # Use simple statistical features as example
    features = []
    
    # Number of genes
    features.append(len(genomic_data))
    
    # Gene variant type statistics
    if 'variant_type' in genomic_data.columns:
        variant_counts = genomic_data['variant_type'].value_counts()
        # Add counts of top 5 most common variant types
        for i in range(5):
            features.append(variant_counts.iloc[i] if i < len(variant_counts) else 0)
    
    # Gene functional impact score statistics
    if 'functional_impact_score' in genomic_data.columns:
        features.append(genomic_data['functional_impact_score'].mean())
        features.append(genomic_data['functional_impact_score'].std())
        features.append(genomic_data['functional_impact_score'].max())
        features.append(genomic_data['functional_impact_score'].min())
    
    # Ensure consistent feature vector dimension
...
# GET WEARABLE DEVICE DATA EMBEDDINGS OF MIMIC IV PATIENT
def get_wearable_embeddings(wearable_data):
    """
    Extract wearable device data embeddings
    
    Args:
        wearable_data: Wearable device data DataFrame
        
    Returns:
        Wearable device embeddings vector
    """
    # If no wearable device data, return zero vector
    if wearable_data is None or len(wearable_data) == 0:
        return np.zeros(50)  # Assume 50-dimensional embedding
    
    # Use time series feature extraction methods
    features = []
    
    # Heart rate features
    if 'heart_rate' in wearable_data.columns:
        hr_data = wearable_data['heart_rate'].dropna()
        if len(hr_data) > 0:
            features.extend([
                hr_data.mean(), hr_data.std(), hr_data.max(), hr_data.min(),
                hr_data.quantile(0.25), hr_data.quantile(0.75)
            ])
        else:
            features.extend([0, 0, 0, 0, 0, 0])
    
    # Step count features
    if 'steps' in wearable_data.columns:
        steps_data = wearable_data['steps'].dropna()
        if len(steps_data) > 0:
            features.extend([
                steps_data.mean(), steps_data.std(), steps_data.max(), steps_data.min(),
                steps_data.sum()  # Total steps
            ])
        else:
            features.extend([0, 0, 0, 0, 0])
    
    # Sleep quality features
    if 'sleep_quality' in wearable_data.columns:
        sleep_data = wearable_data['sleep_quality'].dropna()
        if len(sleep_data) > 0:
            features.extend([
                sleep_data.mean(), sleep_data.std(), sleep_data.max(), sleep_data.min()
            ])
        else:
            features.extend([0, 0, 0, 0])
    
    # Active minutes features
    if 'active_minutes' in wearable_data.columns:
        active_data = wearable_data['active_minutes'].dropna()
        if len(active_data) > 0:
            features.extend([
                active_data.mean(), active_data.std(), active_data.max(), active_data.min()
            ])
        else:
            features.extend([0, 0, 0, 0])
...
# GET PATHOLOGY IMAGE DATA EMBEDDINGS OF MIMIC IV PATIENT
def get_pathology_embeddings(pathology_images):
    """
    Extract pathology image data embeddings
    
    Args:
        pathology_images: Pathology image data DataFrame
        
    Returns:
        Pathology image embeddings vector
    """
    # If no pathology image data, return zero vector
    if pathology_images is None or len(pathology_images) == 0:
        return np.zeros(256)  # Assume 256-dimensional embedding
    
    # In practical applications, pre-trained pathology image recognition models should be used
    # Here we simplify the processing, assuming each image has been converted to a feature vector
    # and stored in the 'features' column
    
    if 'features' in pathology_images.columns:
        # Get the average of all image features
        avg_features = np.mean(np.stack(pathology_images['features'].values), axis=0)
        # Ensure consistent dimensions
        if len(avg_features) < 256:
            padded_features = np.pad(avg_features, (0, 256 - len(avg_features)), 'constant')
            return padded_features
        elif len(avg_features) > 256:
            return avg_features[:256]
        else:
            return avg_features
    else:
        # If no pre-extracted features, return zero vector
        return np.zeros(256)
